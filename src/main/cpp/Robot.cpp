...
void Robot::RobotInit() {
  encoder0 = spark0.GetEncoder();
  encoder1 = spark1.GetEncoder();

  encoder0.SetVelocityConversionFactor(wpi::math::pi*2.0/60.0);
  encoder1.SetVelocityConversionFactor(wpi::math::pi*2.0/60.0);

  controller0 = spark0.GetPIDController();
  controller1 = spark1.GetPIDController();
...
void Robot::TeleopPeriodic() 
{
  ...
  controller0.SetReference(units::radians_per_second_t(reference).to<double>(), 
    rev::ControlType::kVelocity, reference/kMaxSpeed*12.0);

  controller1.SetReference(units::radians_per_second_t(-reference).to<double>(), 
    rev::ControlType::kVelocity, -reference/kMaxSpeed*12.0);
